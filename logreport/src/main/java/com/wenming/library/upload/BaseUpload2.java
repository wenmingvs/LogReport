 ...
 * 抽象的日志报告类
 */
public abstract class BaseUpload2 implements ILogUpload {
    ...
    public BaseUpload2(Context context) {
        mContext = context;
    }
    ...
    @Override
    public void sendFile(final File file, final Service service, final String content) {
        ...
        new Thread(new Runnable() {
            @Override
            public void run() {
                sendReport(buildTitle(mContext), buildBody(mContext, content), file);
                service.stopSelf();
            }
        }).start();
    }
